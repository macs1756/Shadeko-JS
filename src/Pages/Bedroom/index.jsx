...
function BedroomPage({text}) {
	...
	const {data: configurator, loading: loadingConfigurator, error: errorConfigurator} = useFetch(process.env.REACT_APP_API_URL+`/configurator-subcategories?locale=${languageInformation.language}&filters[url][$containsi]=bedroom&populate=*`);
	
	const [skeletonState, setSkeletonState] = React.useState(true);
	const {data: seo, loading: seoLoading, error: seoError}= useFetch(process.env.REACT_APP_API_URL+`/seos?locale=${languageInformation.language}&filters[route]=bedroom&populate=*`);
	...
	return (
		...
		<div className="content">
					<div className="sofas__bread-crumbs d-flex a-i-center">
						...
						<span>/</span>
						<p>{text[0]?.attributes?.breadCrumbCategory2}</p>
					</div>
...
<div className={skeletonState ? "skeleton-load-img active" : "skeleton-load-img"}></div>
	...
	onLoad={()=>{secondDate.length - 1 === index && setSkeletonState(false)}}
	 />
			...
			<Catalog 
			text={text} 
			title={text[0]?.attributes?.catalogTitleBedroom} 
			filterDefault={text[0]?.attributes?.breadCrumbCategory2} 
			filterExp ='&filters[categories][url][$containsi]=Bedroom' 
