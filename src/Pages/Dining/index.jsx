...
function DiningPage({text}) {
   ...
   const {data: seo, loading: seoLoading, error: seoError}= useFetch(process.env.REACT_APP_API_URL+`/seos?locale=${languageInformation.language}&filters[route]=dining&populate=*`);
...
<Helmet>
		...
		<div className="content">
					<div className="sofas__bread-crumbs d-flex a-i-center">
						...
						<span>/</span>
						<p>{text[0]?.attributes?.breadCrumbCategory3}</p>
					</div>
...
<div key={category.id} className="sofas__net-item">
	...
	onLoad={()=>{secondDate.length - 1 === index && setSkeletonState(false)}}
	 />
			...
			<Catalog 
			text={text} 
			title={text[0]?.attributes?.catalogTitleDining}  
			filterDefault={text[0]?.attributes?.breadCrumbCategory3} 
			filterExp ='&filters[categories][url][$containsi]=Dining' 
