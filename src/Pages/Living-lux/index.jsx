...
import '../../Pages/Living/living.scss';
...
function LivingPage({text}) {
	...
	const {data: seo, loading: seoLoading, error: seoError}= useFetch(process.env.REACT_APP_API_URL+`/seos?locale=${languageInformation.language}&filters[route]=living-luxury&populate=*`);
	...
	return (
		...
		<div className="content">
					<div className="sofas__bread-crumbs d-flex a-i-center">
						...
						<span>/</span>
						<p>{text[0]?.attributes?.breadCrumbLuxuryCategory4}</p>
					</div>
...
<div key={category.id} className="sofas__net-item">
	...
	onLoad={()=>{secondDate.length - 1 === index && setSkeletonState(false)}}
	 />
			...
			<Catalog 
			text={text} 
			title={text[0]?.attributes?.catalogTitleLuxuryLiving} 
			filterDefault={text[0]?.attributes?.breadCrumbLuxuryCategory4} 
			filterExp ='&filters[$and][0][categories][url][$containsi]=Luxury-style&filters[$and][1][categories][url][$containsi]=Living&'  
