...
function ConfiguratorLiving({text}) {

  const languageInformation = useSelector(state => state.language.language);

  const canonicalUrl = window.location.href;

  const { data: product, loading: loadingProduct, error: errorProduct } = useFetch(`${process.env.REACT_APP_API_URL}/products?[filters][categories][id]=4&[filters][subcategories][id]=15&populate=*`);
  const { data: productSecond, loading: loadingProductSecond, error: errorProductSecond } = useFetch(`${process.env.REACT_APP_API_URL}/products?[filters][categories][id]=4&[filters][subcategories][id]=17&populate=*`);
  const {data: seo, loading: seoLoading, error: seoError}= useFetch(process.env.REACT_APP_API_URL+`/seos?locale=${languageInformation.language}&filters[route]=configurator-living&populate=*`);
...
<Helmet>
      ...
      <div className="d-flex a-i-center j-c-space">
        ...
        <h2>{text[0]?.attributes?.titleLivingCofigurator}</h2>
      ...
      <div className="configurator__buttons">
        <div 	 
          ...
          <div className="detais">
            <h3>{text[0]?.attributes?.livingConfiguratorTab1Text1}</h3>
            <p>{text[0]?.attributes?.livingConfiguratorTab1Text2}</p>
          </div>
        ...
        <div
          ...
          <div className="detais">
            <h3>{text[0]?.attributes?.livingConfiguratorTab2Text1}</h3>
            <p>{text[0]?.attributes?.livingConfiguratorTab2Text2}</p>
          </div>
        ...
        <div 
          ...
          <div className="detais">
            <h3>{text[0]?.attributes?.livingConfiguratorTab3Text1}</h3>
            <p>{text[0]?.attributes?.livingConfiguratorTab3Text2}</p>
          </div>
        ...
        <h3>{text[0]?.attributes?.livingConfiguratorTab1Title}</h3>
...
{/* 111111111111111111111111111111111111111111 */}
  ...
  loadingProduct ? (
    ...
    {/* 222222222222222222222222222222222222222222 */}
		...
		{activeTab === 2 && (
		<div className="configurator__products">
        <h3>{text[0]?.attributes?.livingConfiguratorTab2Title}</h3>
...
<div className="configurator__navigation-details">
			<h6>{text[0]?.attributes?.livingConfiguratorPanelControlTitle1}</h6>
			<h6 className="d360">{text[0]?.attributes?.livingConfiguratorPanelControlTitle2}</h6>
			<h6 className="d360">{text[0]?.attributes?.livingConfiguratorPanelControlTitle3}</h6>
</div>
			...
			<ul>
					<li className="title">
            <p>{text[0]?.attributes?.livingConfiguratorPanelControlSubtitle1}</p>
            <span>{text[0]?.attributes?.livingConfiguratorPanelControlTitle3}</span>
            </li>
					...
					<li className="price">
            <span className="b360">{text[0]?.attributes?.livingConfiguratorPanelControlTitle2}</span>
			...
			<ul>
					...
					<li className="title">					 
						<p>{text[0]?.attributes?.livingConfiguratorPanelControlSubtitle2}</p>
						<span>{text[0]?.attributes?.livingConfiguratorPanelControlTitle3}</span>
					</li>
			...
			<ul>
						<li className="title">		
						<p>{text[0]?.attributes?.livingConfiguratorPanelControlSubtitle3}</p>
						<span>{text[0]?.attributes?.livingConfiguratorPanelControlTitle3}</span>
						</li>
...
export default ConfiguratorLiving;
