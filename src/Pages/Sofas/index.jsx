...
function SofasPage({text}) {
	...
	const {data: seo, loading: seoLoading, error: seoError}= useFetch(process.env.REACT_APP_API_URL+`/seos?locale=${languageInformation.language}&filters[route]=sofas&populate=*`);
...
<Helmet>
		...
		<div className="content">
					<div className="sofas__bread-crumbs d-flex a-i-center">
						...
						<span>/</span>
						<p>{text[0]?.attributes?.breadCrumbCategory1}</p>
					</div>
...
<div className={skeletonState ? "skeleton-load-img active" : "skeleton-load-img"}></div>
	...
	onLoad={()=>{data.length - 1 === index && setSkeletonState(false)}}
	 />
			...
			<Catalog  
			text={text} 
			title={text[0]?.attributes?.catalogTitleSofas} 
