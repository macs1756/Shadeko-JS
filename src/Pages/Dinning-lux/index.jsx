...
import '../../Pages/Dining/dining.scss';
...
function DiningLux({text}) {
	...
	const {data, loading, error} = useFetch(process.env.REACT_APP_API_URL+`/subcategories?filters[url][$containsi]=dining&locale=${languageInformation.language}&populate=*`);
	const {data: configurator, loadingConfigurator, errorConfigurator}= useFetch(process.env.REACT_APP_API_URL+`/configurator-subcategories?locale=${languageInformation.language}&filters[url][$containsi]=dining&populate=*`);
	const {data: seo, loading: seoLoading, error: seoError}= useFetch(process.env.REACT_APP_API_URL+`/seos?locale=${languageInformation.language}&filters[route]=dining-luxury&populate=*`);
	...
	return (
		...
		<div className="content">
					<div className="sofas__bread-crumbs d-flex a-i-center">
						...
						<span>/</span>
						<p>{text[0]?.attributes?.breadCrumbLuxuryCategory2}</p>
					</div>
	...
	<img
	 ...
	 onLoad={()=>{secondDate.length - 1 === index && setSkeletonState(false)}}
	  />
			...
			<Catalog 
			text={text} 
			title={text[0]?.attributes?.catalogTitleLuxuryDining} 
			filterDefault={text[0]?.attributes?.breadCrumbLuxuryCategory2} 
			filterExp ='&filters[$and][0][categories][url][$containsi]=Luxury-style&filters[$and][1][categories][url][$containsi]=Dining&' 
