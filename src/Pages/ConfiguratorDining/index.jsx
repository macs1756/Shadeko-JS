...
function ConfiguratorDining({text}) {

  const canonicalUrl = window.location.href;

  const languageInformation = useSelector(state => state.language.language);
  const { data: product, loading: loadingProduct, error: errorProduct } = useFetch(`${process.env.REACT_APP_API_URL}/products?[filters][categories][id]=3&[filters][subcategories][id]=12&populate=*`);
  const { data: productSecond, loading: loadingProductSecond, error: errorProductSecond } = useFetch(`${process.env.REACT_APP_API_URL}/products?[filters][categories][id]=3&[filters][subcategories][id]=13&populate=*`);
  const { data: productThree, loading: loadingProductThree, error: errorProductThree} = useFetch(`${process.env.REACT_APP_API_URL}/products?[filters][categories][id]=3&[filters][subcategories][id]=14&populate=*`);
  const {data: seo, loading: seoLoading, error: seoError}= useFetch(process.env.REACT_APP_API_URL+`/seos?locale=${languageInformation.language}&filters[route]=configurator-dining&populate=*`);

  const [modalId,setModalId] = React.useState(92);
...
<Helmet>
      ...
      <div className="d-flex a-i-center j-c-space">
        ...
        <h2>{text[0]?.attributes?.titleDiningCofigurator}</h2>
      ...
      <div className="configurator__buttons">
        <div 	 
          ...
          <div className="detais">
            <h3>{text[0]?.attributes?.diningConfiguratorTab1Text1}</h3>
            <p>{text[0]?.attributes?.diningConfiguratorTab1Text2}</p>
          </div>
        ...
        <div
          ...
          <div className="detais">
            <h3>{text[0]?.attributes?.diningConfiguratorTab2Text1}</h3>
            <p>{text[0]?.attributes?.diningConfiguratorTab2Text2}</p>
          </div>
        ...
        <div 
          ...
          <div className="detais">
            <h3>{text[0]?.attributes?.diningConfiguratorTab3Text1}</h3>
            <p>{text[0]?.attributes?.diningConfiguratorTab3Text2}</p>
          </div>
        ...
        <h3>{text[0]?.attributes?.diningConfiguratorTab1Title}</h3>
...
{/* 111111111111111111111111111111111111111111 */}
  ...
  loadingProduct ? (
    ...
    {/* 222222222222222222222222222222222222222222 */}
		...
		{activeTab === 2 && (
		<div className="configurator__products">
        <h3>{text[0]?.attributes?.diningConfiguratorTab2Title}</h3>
  ...
  {/* 222222222222222222222222222222222222222222 */}
		...
		{activeTab === 3 && (
		<div className="configurator__products">
        <h3>{text[0]?.attributes?.diningConfiguratorTab3Title}</h3>
      ...
      <div onClick={()=>{productClick(el.id)}} className="configurator__products-item">
		...
		{activeTab === 0 && (
			<div className="configurator__preview-photo">
				  <img
                src={process.env.PUBLIC_URL + "/img/categories__bg.jpg"}
                alt="background"
              />
			</div>
		)}
...
<div className="configurator__navigation-details">
			<h6>{text[0]?.attributes?.diningConfiguratorPanelControlTitle1}</h6>
			<h6 className="d360">{text[0]?.attributes?.diningConfiguratorPanelControlTitle2}</h6>
			<h6 className="d360">{text[0]?.attributes?.diningConfiguratorPanelControlTitle3}</h6>
</div>
			...
			<ul>
					<li className="title">
            <p>{text[0]?.attributes?.diningConfiguratorPanelControlSubtitle1}</p>
            <span>{text[0]?.attributes?.diningConfiguratorPanelControlTitle3}</span>
            </li>
					...
					<li className="price">
            <span className="b360">{text[0]?.attributes?.diningConfiguratorPanelControlTitle2}</span>
			...
			<ul>
					...
					<li className="title">					 
						<p>{text[0]?.attributes?.diningConfiguratorPanelControlSubtitle2}</p>
						<span>{text[0]?.attributes?.diningConfiguratorPanelControlTitle3}</span>
					</li>
			...
			<ul>
						<li className="title">		
						<p>{text[0]?.attributes?.diningConfiguratorPanelControlSubtitle3}</p>
						<span>{text[0]?.attributes?.diningConfiguratorPanelControlTitle3}</span>
						</li>
...
export default ConfiguratorDining;
