...
function SofasLux({text}) {
	...
	const {data, loading, error}= useFetch(process.env.REACT_APP_API_URL+`/subcategories?filters[url][$containsi]=sofas&locale=${languageInformation.language}&populate=*`);
	const {data: seo, loading: seoLoading, error: seoError} = useFetch(process.env.REACT_APP_API_URL+`/seos?locale=${languageInformation.language}&filters[route]=sofas-luxury&populate=*`);
...
<div className="content">
					<div className="sofas__bread-crumbs d-flex a-i-center">
						...
						<span>/</span>
						<p>{text[0]?.attributes?.breadCrumbLuxuryCategory1}</p>
					</div>
...
<button onClick={()=>{setFiltersOpen(!filtersOpen)}}>
	...
	<img className={!filtersOpen && 'rotate'} src={process.env.PUBLIC_URL + '/img/header__list-arrow.svg'} alt="arrow" />
				...
				<div className={categoryOpen ? "sofas__net" : 'd-none'}>
...
onLoad={()=>{data.length - 1 === index && setSkeletonState(false)}}
 />
				...
				<Catalog
				 text={text} 
				 title={text[0]?.attributes?.catalogTitleLuxurySofas} 
				 filterDefault={text[0]?.attributes?.breadCrumbLuxuryCategory1} 
				 filterExp ='&filters[$and][0][categories][url][$containsi]=Luxury-style&filters[$and][1][categories][url][$containsi]=Sofas&' 
